...
import { Member, PlanDetail } from "@/lib/api";

export function NotifySidebar({
  plan,
  members,
  meId,
  disabled,
  ...
  const notifyIds = new Set((plan.notifyees || []).map((n) => n.id));
  const candidates = members.filter((m) => !notifyIds.has(m.id));
  ...
  return (
    <aside className="surface rounded-2xl p-5">
            ...
            {candidates.length === 0 ? (
              <div className="space-y-2 px-3 py-3 text-sm text-[var(--muted)]">
                {members.length === 0 ? (
                  ...
                  <p>Everyone in the org is already on the notify list.</p>
                )}
              </div>
            ) : (
      ...
      {(plan.notifyees || []).length === 0 ? (
        <p className="text-sm text-[var(--muted)]">
          No one queued yet. Add teammates (e.g. frontend dev) — they&apos;ll see the Done report
          under <span className="font-medium">Sent to me</span>.
        </p>
      ) : (
        <ul className="space-y-3">
          {(plan.notifyees || []).map((person) => (
            <li key={person.id} className="flex items-center gap-2">
              <Avatar name={person.name} url={person.avatar_url} />
              <div className="min-w-0">
                <p className="truncate text-sm font-medium">{person.name}</p>
                <p className="text-xs text-[var(--muted)]">
                  {plan.status === "done" ? "Notified · Sent to me" : "Will notify on Done"}
                </p>
              </div>
            </li>
          ))}
      ...
      <p className="mt-4 border-t border-[var(--line)] pt-3 text-xs text-[var(--muted)]">
        Add anytime — draft through Done. Shipped plans notify immediately under{" "}
        <span className="font-medium">Sent to me</span>. Outside the org: Anyone share link.
      </p>
    </aside>
  );
}
