...
class PlanlogAPIError(Exception):
...
class PlanlogAPIClient:
    def __init__(
        ...
        self.base_url = (base_url or os.environ.get("PLANLOG_API_URL", "http://localhost:8000")).rstrip("/")
        self.access_token = (
            access_token
            or os.environ.get("PLANLOG_ACCESS_TOKEN")
            or api_key
            or os.environ.get("PLANLOG_API_KEY", "")
        )
        self.organization_id = organization_id or os.environ.get("PLANLOG_ORG_ID", "")
        if not self.access_token:
            raise PlanlogAPIError(
                "Authentication required. Use MCP OAuth login, or set PLANLOG_ACCESS_TOKEN / PLANLOG_API_KEY."
            )
    ...
    def _org_id(self, organization_id: str | None = None) -> str:
        ...
        if not org_id:
            raise PlanlogAPIError(
                "organization_id is required (pass it, or set PLANLOG_ORG_ID, or call list_orgs first)."
            )
        return org_id
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        if response.status_code >= 400:
            ...
            try:
                ...
                pass
            raise PlanlogAPIError(str(detail), response.status_code)
