...
from planlog.auth import generate_api_key, hash_api_key, hash_password
from planlog.config import settings
from planlog.models import DoneHandoff, DoneRecord, Plan, PlanNotifyRequest, PlanReviewRequest, PlanStatus, Suggestion, User
...
def agent_prompt_for_plan(plan: Plan) -> str:
    ...
    if plan.status == PlanStatus.done:
        return (
            f"This Planlog plan is already Done.\n\n"
            f"Plan ID: {plan.id}\n"
            f"Web: {plan_web_url(plan)}\n\n"
