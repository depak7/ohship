"""MCP TokenVerifier — validates OhShip JWTs (and API keys for compatibility)."""

from mcp.server.auth.provider import AccessToken, TokenVerifier

from ohship.auth import try_decode_access_token
from ohship.db import engine
from ohship.oauth.provider import MCP_SCOPE, oauth_provider
from sqlmodel import Session


class OhShipTokenVerifier(TokenVerifier):
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        if token.startswith(("os_", "dl_")):
            from ohship.auth import get_user_by_api_key
