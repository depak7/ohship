...
name: ohship
description: >-
  OhShip Plan → Approve → Done. Use when a plan_id is mentioned, starting feature
...
- Skip OhShip for chores, deps, or formatting with no plan

MCP: `{api_url}/mcp` (OAuth). Search project files for `ohship:begin` for full instructions.
