"""Install OhShip MCP + agent instructions into any project (no OhShip repo required)."""
...
def _template(name: str) -> str:
    return resources.files("ohship.agent.templates").joinpath(name).read_text(encoding="utf-8")
...
def merge_mcp(path: Path, entry: dict) -> None:
    ...
    data.setdefault("mcpServers", {})["ohship"] = entry
...
def stdio_mcp_entry(opts: InstallOptions) -> dict:
    return {
        "command": "uvx",
        "args": ["--from", GIT_PACKAGE, "ohship-mcp"],
        "env": {
            "OHSHIP_API_URL": opts.api_url.rstrip("/"),
            "OHSHIP_API_KEY": opts.api_key,
            "OHSHIP_ORG_ID": opts.org_id,
        },
    }
...
def install_instructions(opts: InstallOptions) -> list[str]:
    ...
    print("Grafting OhShip instructions…")
...
def install_cursor_mcp(opts: InstallOptions) -> list[str]:
    ...
    msg = f"  {mcp_path} — ohship MCP merged"
    lines.append(msg)
    print(msg)

    rules_dir = opts.repo / ".cursor" / "rules"
    rules_dir.mkdir(parents=True, exist_ok=True)
    rule_dst = rules_dir / "ohship.mdc"
    rule_text = _template("ohship.mdc")
...
def install_claude_mcp(opts: InstallOptions) -> list[str]:
    ...
    merge_mcp(mcp_path, entry)
    msg = f"  {mcp_path} — ohship MCP merged (Claude Code)"
    print(msg)
    return [msg]


def install_global_skill(opts: InstallOptions) -> list[str]:
    skill_dir = Path.home() / ".cursor" / "skills" / "ohship"
...
def run_install(opts: InstallOptions) -> int:
    ...
    print()
    print("OhShip agent setup complete.")
    print()
    print("Any coding agent:")
    print('  Search for "ohship:begin" in AGENTS.md / CLAUDE.md in this project.')
    print("  Before coding on a plan: get_plan. When shipped: post_done.")
    print()
    if "cursor" in active:
        print("Cursor: reload MCP, authenticate ohship (OAuth browser flow).")
    if "claude" in active:
        print("Claude Code: reload or run `claude mcp list` — authenticate ohship.")
    print()
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ohship-agent",
        description="Wire OhShip MCP + instructions for any coding agent (no repo clone required).",
    )
    p.add_argument(
        "command",
        nargs="?",
    ...
    p.add_argument("--api-url", default=os.environ.get("OHSHIP_API_URL", DEFAULT_API_URL))
    ...
    p.add_argument("--api-key", default=os.environ.get("OHSHIP_API_KEY", ""))
    p.add_argument("--org-id", default=os.environ.get("OHSHIP_ORG_ID", ""))
    p.add_argument("--all-agents", action="store_true", help="Create all standard agent MD files")
    p.add_argument("--no-global-skill", action="store_true", help="Skip ~/.cursor/skills/ohship")
